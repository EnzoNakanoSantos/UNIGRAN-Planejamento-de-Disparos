-- Correções gerais do projeto no Supabase.
-- Execute este arquivo no SQL Editor quando aparecer erro de coluna ausente,
-- RLS/permissão, status inválido, tabela ausente ou campo antigo obrigatório.
...
ALTER TABLE public.base_rules
ADD COLUMN IF NOT EXISTS spreadsheet_attachment JSONB;
...
ALTER TABLE public.dispatches
ADD COLUMN IF NOT EXISTS template_name TEXT DEFAULT '';

ALTER TABLE public.dispatches
ADD COLUMN IF NOT EXISTS chip VARCHAR(10) DEFAULT '';

ALTER TABLE public.dispatches
ADD COLUMN IF NOT EXISTS html_content TEXT DEFAULT '';
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'base_rules'
      AND column_name = 'conversion_criterion'
  ) THEN
    ALTER TABLE public.base_rules
    ALTER COLUMN conversion_criterion DROP NOT NULL;

    ALTER TABLE public.base_rules
    ALTER COLUMN conversion_criterion SET DEFAULT '';

    UPDATE public.base_rules
    SET conversion_criterion = ''
    WHERE conversion_criterion IS NULL;
  END IF;
END $$;

UPDATE public.dispatches
SET status = 'Em produção'
WHERE status = 'Em producao';

ALTER TABLE public.dispatches
DROP CONSTRAINT IF EXISTS dispatch_chip_check;

ALTER TABLE public.dispatches
ADD CONSTRAINT dispatch_chip_check CHECK (
  chip IN ('', 'EAD', 'DOU', 'CGR', 'U.S.A')
);

ALTER TABLE public.dispatches
DROP CONSTRAINT IF EXISTS dispatch_channel_check;

ALTER TABLE public.dispatches
ADD CONSTRAINT dispatch_channel_check CHECK (
...
SELECT
  ...
  ARRAY_REMOVE(ARRAY[
    CASE WHEN TRIM(COALESCE(b.main_base, '')) = '' THEN 'Base principal não informada' END,
    CASE WHEN TRIM(COALESCE(b.excluded_bases, '')) = '' THEN 'Regra de exclusão não configurada' END,
    CASE WHEN b.last_updated IS NULL THEN 'Data de atualização não preenchida' END
  ], NULL) AS issues
...
SELECT
  ...
  ARRAY_REMOVE(
    ARRAY[
      CASE WHEN TRIM(COALESCE(d.audience, '')) = '' THEN 'Público não definido' END,
      CASE WHEN b.id IS NULL THEN 'Base principal não informada' END,
...
ALTER TABLE public.base_rules
DROP COLUMN IF EXISTS auto_exclude;

ALTER TABLE public.base_rules
DROP COLUMN IF EXISTS auto_condition;
